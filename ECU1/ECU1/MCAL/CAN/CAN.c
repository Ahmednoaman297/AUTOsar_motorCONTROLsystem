 ...
 ****************************************************************/
uint8_t CANTransmit(uint8_t a_moduleIndex, uint8_t a_objectID, uint8_t *a_pui8Data, uint32_t a_ui32MsgLen, bool isRemote)
{
    uint8_t Error_transmit = CAN_NO_ERROR;

    if (a_ui32MsgLen > (uint32_t)8 && !isRemote) {
        ...
        if ((uint8_t)Zero_CAN == error) {
            ...
            msgObject.ui32MsgLen = isRemote ? 0 : a_ui32MsgLen;  // Remote frames have no data
            msgObject.pui8MsgData = isRemote ? NULL : a_pui8Data; // No data for remote frames

            // Set the message type
            uint32_t msgType = isRemote ? MSG_OBJ_TYPE_TX_REMOTE : MSG_OBJ_TYPE_TX;
            CANMessageSet((uint32_t)CAN_Modules[a_moduleIndex].Module, (uint32_t)a_objectID, &msgObject, msgType);
        } else {
            Error_transmit = CAN_ERROR_TRANSMIT;
        }
    }
...
//    uint8_t error = Zero_CAN;
//
//    for (uint8_t i = (uint8_t)Zero_CAN; i < CAN_Modules[a_moduleIndex].numObjects; i++) {
//        if (CAN_Modules[a_moduleIndex].objects[i].objectID == a_objectID) {
//            error = (uint8_t)Zero_CAN;
//        }
//    }
...
//        CANMessageGet((uint32_t)CAN_Modules[a_moduleIndex].Module, (uint32_t)a_objectID, &msgObject, true);
//
//        /* Check if the message is a Remote Frame */
//        if (msgObject.ui32Flags & MSG_OBJ_REMOTE_FRAME) {
//            // Handle Remote Frame
//            uint8_t responseData[8] = {0x55, 0xAA}; // Example data
//            CANTransmit(a_moduleIndex, a_objectID, responseData, 2, false); // Respond with Data Frame
//            return_variable = 0; // No data returned for Remote Frame
//        } else {
//            return_variable = msgObject.ui32MsgLen; /* Return the length of the received message */
//        }
//    } else {
//        return_variable = CAN_ERROR_RECEIVE;
//    }
//
//    return return_variable;
//}
