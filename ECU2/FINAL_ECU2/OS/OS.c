...
void OS_Init(void)
{
    ...
    for (i = ZERO; i < NUMBER_OF_tasks; i++) {
        ...
        taskList[i].isSuspended = (uint8_t)0;  /* Initialize all tasks as not suspended */
    }
}
...
void OS_Stop(void)
{
    osRunning = (uint8_t)0;  /* Explicitly cast to uint8_t */
}
...
void OS_Resume(void)
{
    osRunning = (uint8_t)1;  /* Explicitly cast to uint8_t */
}
...
void OS_SuspendTask(uint8_t a_taskIndex)
{
    if (a_taskIndex < NUMBER_OF_tasks) {
        taskList[a_taskIndex].isSuspended = (uint8_t)1;  /* Explicitly cast to uint8_t */
...
{
    if (a_taskIndex < NUMBER_OF_tasks) {
        taskList[a_taskIndex].isSuspended = (uint8_t)0;  /* Explicitly cast to uint8_t */
    }
}
